    ...
    >
      ...
      <form class="space-y-4 md:space-y-6">
        ...
        <div>
          ...
          <p v-if="firstNameError" class="text-red-500 text-xs mt-2 ml-1">
            {{ firstNameError }}
          </p>
        </div>
        ...
        <div>
          ...
          <p v-if="lastNameError" class="text-red-500 text-xs mt-2 ml-1">
            {{ lastNameError }}
          </p>
        </div>
        ...
        <div>
          ...
          <div
            :class="[
              'input input-bordered flex items-center gap-2 bg-white w-full',
              { 'input-error': repeatPasswordError },
            ]"
          >
            <input
              :type="showPassword ? 'text' : 'password'"
          ...
          <p v-if="repeatPasswordError" class="text-red-500 text-xs mt-2 ml-1">
            {{ repeatPasswordError }}
          </p>
        </div>
...
export default {
  setup() {
    ...
    // Fungsi handleSubmit yang mengirim data ke API
    const handleSubmit = async () => {
      try {
        ...
        Swal.fire({
          ...
          // Mengarahkan ke halaman login setelah pengguna menekan OK
          router.push('/login');
        });
    ...
    const login = async () => {
      ...
      if (!firstNameError.value && !emailError.value && !passwordError.value) {
        ...
        await handleSubmit(); // Menunggu hingga handleSubmit selesai
